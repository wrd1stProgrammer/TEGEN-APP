/*  ────────────────────────────────────
    ...
    const AD_UNIT_TEST_ANDROID = 'ca-app-pub-3940256099942544/5224354917';
    ...
    export default function MediaSelectionScreen({ route }: Props) {
      ...
      const [loadingPhase, setLoadingPhase] = useState<'ad' | 'analysis'>('ad');
      ...
      const geminiCall = async (uri: string) => {
        ...
        try {
          ...
          navigate('FaceResultScreen', { scores: res?.data, sex, photoUri: uploaded });
        } catch {
          Alert.alert(t.errorText);
        } finally {
          setLoading(false);
        }
      };
        ...
        <SafeAreaView style={styles.safe}>
          ...
          <View style={styles.content}>
            ...
            {selectedImage && (
              <View style={styles.previewBox}>
                ...
                <TouchableOpacity onPress={() => setSelectedImage(null)} style={styles.removeBtn} hitSlop={15}>
            ...
            >
              <Ionicons name="sparkles" size={22} color={selectedImage ? '#fff' : '#bbb'} style={{ marginRight: 7 }} />
              <Text style={[styles.analyzeText, { color: selectedImage ? '#fff' : '#bbb' }]}>​
                {loading
                  ? loadingPhase === 'ad'
                    ? t.loadingAdText
          ...
          >
            <View style={styles.modalBox}>
              <Ionicons name="gift-outline" size={36} color="#FC3468" style={{ marginBottom: 12 }} />
          ...
          {loading && (
            <View style={styles.loadingOverlay} pointerEvents="none">
              ...
              <Text style={styles.loadingText}>
                {loadingPhase === 'ad' ? t.loadingAdText : t.loadingAnalysisText}
              </Text>
            </View>
          )}
        </SafeAreaView>
    ...
    const styles = StyleSheet.create({
      ...
      headerRow: { flexDirection: 'row', alignItems: 'center', height: Platform.OS === 'ios' ? 56 : 48, paddingHorizontal: 6 },
      backIconBtn: { padding: 8, width: 40, alignItems: 'center' },
    
      content: { flex: 1, justifyContent: 'center', alignItems: 'center', marginBottom: 80, paddingHorizontal: 18 },
      title: { fontSize: 24, fontWeight: 'bold', color: '#111', textAlign: 'center', marginBottom: 36, lineHeight: 34 },
    
      buttonGroup: { width: '80%', flexDirection: 'row', gap: 22, marginBottom: 30, justifyContent: 'center' },
      galleryBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: '#111', borderRadius: 25, paddingVertical: 18, justifyContent: 'center', marginRight: 7 },
      galleryText: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
      cameraBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', borderRadius: 25, borderWidth: 1.5, borderColor: '#111', paddingVertical: 18, justifyContent: 'center', marginLeft: 7 },
      cameraText: { color: '#111', fontSize: 18, fontWeight: 'bold' },
    
      previewBox: { marginTop: 26, marginBottom: 18, width: 145, height: 145, borderRadius: 19, overflow: 'hidden', borderWidth: 1.5, borderColor: '#f3c8e0', backgroundColor: '#f8f8fa', alignItems: 'center', justifyContent: 'center', position: 'relative' },
      ...
      analyzeBtn: { flexDirection: 'row', alignItems: 'center', borderRadius: 26, marginTop: 10, marginBottom: 18, paddingVertical: 16, paddingHorizontal: 38 },
      analyzeBtnActive: { backgroundColor: '#FC3468' },
      analyzeBtnInactive: { backgroundColor: '#e8e7e7' },
      analyzeText: { fontWeight: 'bold', fontSize: 18 },
    
      tip: { color: '#B3B3B3', fontSize: 14, marginTop: 10, textAlign: 'center', fontWeight: '500' },
    
      modalBox: { backgroundColor: '#fff', borderRadius: 20, paddingVertical: 36, paddingHorizontal: 30, alignItems: 'center' },
      modalTitle: { fontSize: 19, fontWeight: 'bold', color: '#222', marginBottom: 10 },
      modalDesc: { fontSize: 16, color: '#444', textAlign: 'center', lineHeight: 22, marginBottom: 18 },
      modalBtns: { flexDirection: 'row', width: '100%', justifyContent: 'space-between' },
      modalBtnCancel: { flex: 1, backgroundColor: '#eee', borderRadius: 11, marginRight: 7, paddingVertical: 12, alignItems: 'center' },
      modalBtnOk: { flex: 1, backgroundColor: '#FC3468', borderRadius: 11, marginLeft: 7, paddingVertical: 12, alignItems: 'center' },
      modalBtnTextCancel: { color: '#555', fontSize: 16, fontWeight: 'bold' },
      modalBtnTextOk: { color: '#fff', fontSize: 16, fontWeight: 'bold' },
    
      loadingOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(255,255,255,0.55)', justifyContent: 'center', alignItems: 'center', zIndex: 20 },
      loadingText: { marginTop: 8, fontSize: 14, color: '#FC3468', fontWeight: '500' },
    });
