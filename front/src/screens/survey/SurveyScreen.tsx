...
const getQuestions = (gender: '남' | '여') => [
  ...
  {
    category: '연애 성향',
    question: `${gender === '남' ? '여자친구' : '남자친구'}가 ${gender === '남' ? '남사친' : '여사친'}과 술을 먹는다고 한다. 나는?`,
    options: [
      '불편하다, 싫다고 말한다',
      '조용히 신경 쓴다',
      '괜찮다, 믿는다',
...
interface Props {
  route: { params: { gender: '남' | '여' } };
}

export default function SurveyScreen({ route }: Props) {
  const gender = route.params?.gender ?? '남';
  ...
  const handleSelect = (idx: number) => {
    ...
    if (step < questions.length - 1) {
      ...
      // 답변 → 결과 수치로 가공
      const score = calculateTegenScore(answers.concat(idx), gender);
      navigate('Face', { answers: answers.concat(idx), gender, score });
    }
  };
